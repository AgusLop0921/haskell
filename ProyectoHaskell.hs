...
procesarEstado (Just (Estado symTable)) cinta posicion = handleAddingTape (procesarInstruccion (H.lookup (cinta!!posicion) symTable) cinta posicion)
...
handleAddingTape (cinta, proximoEstado, posicion)
...
loop i maquina alfabeto cinta estadoInicial posicion =
                           do  putStr ("Estado " ++ show i ++ ". Cinta: [" ++ mostrarCinta cinta posicion "" 0 ++ "]" ++ "\n" ++ "Estado " ++ show i ++ ". En estado " ++ show estadoInicial ++ "\n"
                                        ...
                                        ++ "Estado " ++ show i ++ ". " ++ showInstruction maquina alfabeto cinta estadoInicial posicion ++ "\n\n")
                               let (nuevaCinta, nuevoEstado, newPosition) = ejecutarPaso maquina alfabeto cinta estadoInicial posicion in
                                   ...
                                   else loop (i+1) maquina alfabeto nuevaCinta nuevoEstado newPosition
...
addToPrintStr :: Maybe TMInstruccion -> String -> String -> String
addToPrintStr (Just (Instruccion simbolo dir proximoEstado)) readSym str = (str ++ "(read " ++ show readSym ++ ", write " ++ show simbolo ++ ", movimiento " ++ show dir ++ ", proximo estado: " ++ show proximoEstado ++ ") ")
addToPrintStr Nothing readSym str = str

showInstruction maquina alfabeto cinta estadoInicial posicion = showInstructionHelper (H.lookup estadoInicial maquina) cinta posicion

showInstructionHelper (Just (Estado symTable)) cinta posicion = (showInstructionHelper2 (H.lookup (cinta!!posicion) symTable) (cinta!!posicion))

showInstructionHelper2 (Just (Instruccion escribirSimbolo dir proximoEstado)) readSymbol = ("Instruccion: ( Leyendo  " ++ show readSymbol ++ ", write " ++ show escribirSimbolo ++ ", movimiento " ++ show dir ++ ", proximo estado: " ++ show proximoEstado ++ ")")
